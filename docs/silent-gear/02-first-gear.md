...
```
재료 아이템에 마우스 올리기 + 재료 정보 표시 키 누르기
→ 스탯 수치, 특성(Trait) 전체 표시
```

기본 키 후보는 **Ctrl**입니다. 다르게 동작하면 `Controls`에서 `silent`, `gear`, `tooltip`으로 검색해 확인하세요.
...
**재료 정보 키를 눌러도 스탯이 안 나와요.**
→ `Controls`에서 Silent Gear 관련 툴팁 키를 먼저 확인하세요.
→ 키가 맞는데도 안 보이면 해당 아이템이 Silent Gear 소재로 등록되지 않았을 수 있습니다.
