...
sidebar_position: 3
description: AE2에서 가장 헷갈리는 채널(Channel) 시스템의 작동 원리와 올바른 네트워크 설계 방법을 이해합니다.
...
채널이 활성화된 기본 설정 기준입니다. 서버 설정에 따라 채널 기능이 비활성화되거나 조정될 수 있습니다.
...
| 구조 | 설명 |
|------|------|
| 권장: 트리 구조 | Controller에서 Dense Cable 간선을 뻗고, 저장 구역/접근 구역/자동화 구역을 가지처럼 분리 |
| 비권장: 루프 구조 | Controller와 기기가 다시 Controller로 이어지는 폐쇄 루프. 채널 경로가 예측하기 어려워짐 |

### Dense 케이블을 간선으로 사용

| 구간 | 추천 케이블 | 역할 |
|------|-------------|------|
| Controller에서 나가는 메인 간선 | Dense Cable | 32채널 전달 |
| 각 구역으로 갈라지는 분기 | 일반 Cable | 구역별 최대 8채널 전달 |
| 기기 연결부 | 일반 Cable 또는 Smart Cable | 실제 기기 연결, 채널 사용량 확인 |
