...
```mermaid
flowchart TD
  A[오스뮴 대량 확보] --> B[Metallurgic Infuser<br/>강철 생산 시작]
  B --> C[Heat Generator<br/>첫 에너지 확보]
  C --> D[Enrichment Chamber + Smelter<br/>광석 2배 처리]
  D --> E[Pump + Electrolytic Separator<br/>산소 생산]
  E --> F[Purification Chamber + Crusher<br/>광석 3배 처리]
  F --> G[Thermal Evaporation Plant<br/>광석 4배 처리]
  G --> H[Factory 기계<br/>처리량 증가]
  H --> I[Digital Miner<br/>자동 채굴]
  I --> J[QIO 저장 시스템]
  J --> K[MekaSuit<br/>최강 장비 완성]
```
